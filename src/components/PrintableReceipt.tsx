...
export const PrintableReceipt = ({ account, customer, transaction }: { transaction: Transaction; customer: Customer; account: Account }) => (
  <div className="mx-auto max-w-2xl rounded-3xl border border-slate-200 bg-white p-8 text-slate-900 print:border-0 print:shadow-none" id="printable-receipt">
    <div className="border-b border-slate-200 pb-6"><p className="text-xs font-semibold uppercase tracking-[0.3em] text-blue-700">ITMANAGEMENT.COM</p><h2 className="mt-2 text-2xl font-semibold">Bank Transaction Receipt</h2><p className="mt-1 text-sm text-slate-500">Generated for {customer.name}</p></div>
    ...
    <p className="mt-6 text-center text-xs text-slate-500">Thank you for banking with ITMANAGEMENT.COM. This receipt is system generated.</p>
  </div>
)
